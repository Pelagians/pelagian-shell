...
`container-smoke` builds the image, starts its real `/init`, waits for Labwc, layoutd, the session sentinel, and Selkies HTTPS, then uses native GTK/Wayland fixtures to prove:
